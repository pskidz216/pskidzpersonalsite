...
(function(){
  ...
  + '@media(max-width:1050px){.brand--lockup img{height:52px}}'
  + '@media(max-width:1260px){.nav .btn{margin-left:auto}}'
  ...
  /* every Schedule Your Estimate button goes to the estimate page; on that page, to its form */
  var estimate = document.getElementById('estimate-form') ? '#estimate-form' : 'contact.html';
  ...
  + '<div class="dmisc"><a href="reviews.html">Reviews</a><a href="faq.html">FAQ</a><a href="contact.html">Contact</a><a href="neighborhoods.html">Neighborhoods A&ndash;Z</a></div>'
  + '</nav>'
  ...
  /* balance the lockup between the menu button and the right-hand cluster */
  (function(){
    ...
    if (!lockup) return;
    var place = function(){
      if (window.innerWidth < 901) { lockup.style.left = ''; lockup.style.transform = ''; return; }
      var menu = nav.querySelector('.menu-btn') || nav.querySelector('.nav__links');
      var tel = nav.querySelector('.nav__tel');
      var right = tel && getComputedStyle(tel).display !== 'none' ? tel : nav.querySelector('.btn');
      if (!menu || !right) return;
      var nr = nav.getBoundingClientRect();
      var mid = ((menu.getBoundingClientRect().right - nr.left) +
                 (right.getBoundingClientRect().left - nr.left)) / 2;
      lockup.style.left = mid + 'px';
      lockup.style.transform = 'translate(-50%,-50%)';
    };
    place();
